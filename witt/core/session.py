...
class AppSession:
    ...
    @property
    def before(self):
        return self.ctx.config["logic"]["before"]

    @property
    def after(self):
        return self.ctx.config["logic"]["after"]

    @property
    def executor(self):
        return DockerAdapter(self.ctx) if self.ctx.config["env"].get("mode") != 3 else SSHAdapter(self.ctx.config)

    @property
    def recorder(self):
        return Recorder(self.executor)
    ...
    def record_compress(self, record_path: Path):
        ...
        selected_indices = parser.get_selected_indices(channels)
    ...
    def task_play(self):
        ...
        while True:
            ...
            print(f"\n{'ID '} | {self.ctx.vehicle:<9} | {self.ctx.target_date}")
            print("-" * 65)
            count = 1
            for entry in library:
                if (
                    ...
                    indent = " " * 4
                    soc1_last_update = entry["fast_meta"]["last_update"].get("soc1", "")
                    soc2_last_update = entry["fast_meta"]["last_update"].get("soc2", "")
                    soc1_info = f"soc1  update: \033[0;33m{soc1_last_update}\033[0m"
                    soc2_info = f"soc2  update: \033[1;33m{soc2_last_update}\033[0m"
                    print(
                        f"{indent}├── {soc1_info}"
                    )
                    print(f"{indent}└── {soc2_info}")
                    count += 1
            tag_idx = input("\n请选择播放序号 (回车取消): ").strip()
            if not tag_idx:
                return
