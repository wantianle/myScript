...
class WorkflowTests(unittest.TestCase):
    ...
    def test_slice_progress_shows_result_when_download_plan_is_empty(self) -> None:
        ...
        with patch("interface.workflow._load_task_entries", return_value=[task_entry]):
            with patch("interface.workflow.prompter.get_selected_indices", return_value=[task_entry]):
                with patch("interface.workflow.prompter_channel.get_paths_channels", return_value=[]):
                    with patch("interface.workflow.ui.show_result_section") as show_result_section:
                        workflow.slice_progress(session)
    ...
    def test_search_flow_calls_prompt_collectors_and_runner(self) -> None:
        ...
        with patch("interface.workflow.prompter_config.get_basic_params") as get_basic_params:
            with patch("interface.workflow.prompter_config.get_source_path_params") as get_source_path_params:
                with patch("interface.workflow.prompter_config.get_export_path_params") as get_export_path_params:
                    with patch("interface.workflow.prompter_config.get_split_params") as get_split_params:
    ...
    def test_search_flow_shows_result_when_find_record_script_fails(self) -> None:
        ...
        with patch("interface.workflow.prompter_config.get_basic_params"):
            with patch("interface.workflow.prompter_config.get_source_path_params"):
                with patch("interface.workflow.prompter_config.get_export_path_params"):
                    with patch("interface.workflow.prompter_config.get_split_params"):
                        with patch("interface.workflow.ui.show_result_section") as show_result_section:
