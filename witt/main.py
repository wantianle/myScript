...
def main_menu():
    while True:
        ...
        if choice == "1":
            cli.get_basic_info(session.ctx.config)
            cli.get_path_params(session.ctx.config)
            try:
