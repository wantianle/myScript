...
_merge_config_restart() {
    ...
    # 2. 【核心修改部分】
    # 这里的逻辑从“合并”变成了“直接覆盖”。
    # 直接读取 Mixin 文件内容，写入到运行时配置文件中。
    sudo cat "$CLASH_CONFIG_MIXIN" | sudo tee "$CLASH_CONFIG_RUNTIME" >/dev/null

    # 3. 验证配置有效性
    # 如果你的 Mixin 文件写错了，这里会报错并还原备份，防止断网。
    _valid_config "$CLASH_CONFIG_RUNTIME" || {
        sudo cat $backup | sudo tee "$CLASH_CONFIG_RUNTIME" >/dev/null
        _error_quit "验证失败：您的 Mixin 配置格式不正确，已回滚。"
    }
